...
const CACHE_VERSION = 'farma-v14-20260812';
...
self.addEventListener('fetch', function(event) {
  ...
  // HTML / navegação: sempre rede (sem cache HTTP), atualiza shell em segundo plano
  var isNav = req.mode === 'navigate' || /\/index\.html$/i.test(url.pathname) || url.pathname === '/' || url.pathname.endsWith('/');
  if (url.origin === self.location.origin) {
    event.respondWith(
      fetch(req, isNav ? { cache: 'no-store' } : undefined).then(function(res) {
        if (res && res.ok && !isConfig(url) && !isNav) {
          var copy = res.clone();
          caches.open(SHELL_CACHE).then(function(cache) {
            try { cache.put(req, copy); } catch (e) {}
          });
        }
        // Atualiza shell do index em background para próxima visita offline
        if (res && res.ok && isNav) {
          var copy2 = res.clone();
          caches.open(SHELL_CACHE).then(function(cache) {
            try {
              cache.put('./index.html', copy2.clone());
              cache.put(req, copy2);
            } catch (e) {}
          });
        }
