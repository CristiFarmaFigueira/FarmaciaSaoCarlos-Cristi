        ...
        /** Aceita só http(s) ou data:image/ — evita javascript: e outros esquemas */
        function safeImageUrl(src) {
            if (src == null) return '';
            var s = String(src).trim();
            if (!s) return '';
            if (s.indexOf('data:image/') === 0) {
                // data URL de imagem (upload local)
                return s;
            }
            try {
                ...
                if (!window.__farmaImgIO) {
                    window.__farmaImgIO = new IntersectionObserver(function(entries) {
                        entries.forEach(function(entry) {
                            ...
                            if (real) {
                                var safe = (typeof safeImageUrl === 'function') ? safeImageUrl(real) : real;
